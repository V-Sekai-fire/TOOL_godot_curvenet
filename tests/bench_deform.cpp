...
//
// Wall-clock benchmark for the DeGoes22 deformer's dense LU path at a
// few mesh sizes. Captures the baseline pre-sparse-CG so subsequent
// perf work has something to compare against. See
// `docs/PERF_BASELINE.md` for the recorded numbers.
//
...
BenchRow run_one(int n, int frames) {
	...
	// Promote the four corner mesh-vertices to sample columns 0..3.
	...
	// Per-frame: identity Fc (4 samples × 9 entries), varying Xc.
...
int main(int argc, char ** /*argv*/) {
	(void)argc;
	std::printf("DeGoes22 deformer baseline — dense LU factor-once path\n");
	std::printf("\n");
	...
	for (const int n : {10, 15, 20, 25, 30}) {
		const BenchRow row = run_one(n, frames);
		const double fps = 1000.0 / row.frame_ms_avg;
