...
//
// Two-level Schwarz preconditioner training run on Mire 5k.
//
...
int main(int argc, char **argv) {
    (void)argc; (void)argv;
    const double T_CAP_MS = 30000.0;   // hard 30 s wall-clock cap
    ...
    if (converged) {
        std::printf("\nTwo-level Schwarz: converged in %zu outer iters\n",
                      iters_done);
        std::printf("vs 1-level Chebyshev-Schwarz at 5k: 137 outer iters\n");
        if (iters_done > 0) {
            const double ratio = 137.0 / static_cast<double>(iters_done);
