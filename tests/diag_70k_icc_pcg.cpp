...
//
// ICC(0)-preconditioned CG on the 81k Mire cut-mesh Laplacian.
//
...
// 200,000 iters / 133 s -> ‖r‖² = 3.6e-10. Goal: ICC(0) cuts that
...
int main() {
    ...
    if (st.converged) {
        ...
        std::printf("vs plain CG baseline: 200,000 iters / 133 s\n");
        if (st.iters > 0) {
            std::printf("speedup: %.0fx in iter count, %.0fx in wall time\n",
                          200000.0 / st.iters, 133000.0 / t_solve);
        }
    } else {
